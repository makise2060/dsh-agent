...
<div class="h-full overflow-y-auto bg-gray-50 p-6">
  <div class="mx-auto max-w-2xl space-y-6">
    <h1 class="text-lg font-bold text-gray-800">版本与更新</h1>

    <!-- App self update -->
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <div class="flex items-center justify-between">
        <span class="text-sm font-medium text-gray-700">DSH Agent (本应用)</span>
        <span class="text-xs text-gray-400">v1.0.0</span>
      </div>
      <div class="mt-3">
        {#if checkingApp}
          <span class="text-xs text-gray-400">检查中...</span>
        {:else if appUpdate?.update_available}
          <p class="text-xs text-orange-600">最新版本: {appUpdate.latest_version}</p>
        {:else if appUpdate}
          <span class="text-xs text-green-600">已是最新版本</span>
        {/if}
      </div>
      <button
        class="mt-3 rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50"
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <div class="flex items-center justify-between">
        ...
        {#if dsh.installed}
          <span class="text-xs text-gray-400">v{dsh.version}</span>
        {:else}
          <span class="text-xs text-red-500">未安装</span>
        {/if}
      </div>
      <div class="mt-3">
        {#if checkingDsh}
